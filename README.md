...
4. Follow the prompts: enter the first speaker's name, then after each turn press ENTER to keep the same speaker, type a new name to switch speaker, or type done to finish recording. The pipeline then runs correction, enrichment, and validation, writing the CSVs to data/.
...
Uses .apply() to visit each of the n rows once and check whether its text ends with a question mark. One constant-time string check per row → n × O(1) = O(n). Stores one boolean per row → O(n) space.

**2. num_words — O(n) time, O(n) space.**
.apply() visits each of the n rows once and splits text on whitespace to count words. Sentence length is bounded and small, so it is treated as constant per row. n rows × O(1) = O(n). One integer per row → O(n) space.
